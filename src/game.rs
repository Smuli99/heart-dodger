...
impl Game {
    ...
    fn update(&mut self) {
        // move player if not colliding with wall
        let player_next_position = self.player.forward_position();
        if !self.walls.iter().any(|wall| wall.position() == player_next_position) {
        ...
        if self.player.position().round().to_u16() == self.collectible.position() {
            ...
            while self.walls.iter().any(|wall| wall.position() == self.collectible.position()) {
                self.collectible.set_rand_position(
                    &mut self.rng,
                    1..self.width - 1,
                    1..self.height - 1,
        ...
        // move enemies
        self.enemies.iter_mut().for_each(|enemy| enemy.move_towards_player(&self.player));
